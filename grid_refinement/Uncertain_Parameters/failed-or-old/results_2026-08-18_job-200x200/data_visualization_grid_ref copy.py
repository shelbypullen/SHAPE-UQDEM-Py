...
print(np.nanmin(total_KE_avgs[-1]))
# norm2 = colors.TwoSlopeNorm(vmin=np.log10(np.nanmin(total_KE_avgs[-1])), 
#                             vcenter=np.log10(10), 
#                             vmax=np.log10(np.nanmax(total_KE_avgs[-1])))
pcm2 = axs2[0].pcolormesh(c3s,c2s, total_KE_avgs[-1],cmap='RdBu_r', norm=colors.LogNorm(vmin=0.3, vmax=3))#,norm=norm2)
...
cbar2.set_ticks([0.3,0.4,0.6,1,2,3])
cbar2.set_ticklabels(["0.3"," "," ","1"," ","3"])
...
pcm3 = axs2[1].pcolormesh(c3s,c2s, total_KE_stds[-1],cmap='RdBu_r',norm=colors.LogNorm(vmin=0.03, vmax=0.3))
...
cbar3.set_ticks([0.03,0.04,0.06,0.1,0.2,0.3])
cbar3.set_ticklabels(["0.03"," "," ","0.1"," ","0.3"])
...
pcm2 = axs2[2].pcolormesh(c3s,c2s, total_KE_cost[-1],cmap='RdBu_r',norm=colors.LogNorm(vmin=0.3, vmax=3))
...
cbar4.set_ticks([0.3,0.4,0.6,1,2,3])
cbar4.set_ticklabels(["0.3"," "," ","1"," ","3"])
