...
def objective(non_spring_coeff_info, stochastic_info, n_samples):
    ...
    for k in range(n_samples):
        ...
        # Solving ODE
        ############################################################
        non_rhs = lambda t, x, **kwargs: np.asarray(EOM.impact_EOM(t, x, non_spring_info, mat_info, 0))
        non_ode_system = de.OdeSystem(
            non_rhs,
            y0=np.asarray(init_vals),
            dense_output=True,
            t=(0,T),
            dt=dt,
            rtol = 1e-12,
        ...
        non_ode_system.method = "RK1412"
        non_ode_system.integrate()

        X_non = np.array([np.asarray(non_ode_system.sol(ti)) for ti in t_eval])
        t_non = np.asarray(t_eval)

        x_dot_non = X_non[:,1::2]
        x_non = X_non[:,0::2]

        lin_rhs = lambda t, x, **kwargs: np.asarray(EOM.impact_EOM(t, x, lin_spring_info, mat_info, 0))
        lin_ode_system = de.OdeSystem(
            lin_rhs,
            y0=np.asarray(init_vals),
            dense_output=True,
            t=(0,T),
        ...
        lin_ode_system.method = "RK1412"
        lin_ode_system.integrate()

        X_lin = np.array([np.asarray(lin_ode_system.sol(ti)) for ti in t_eval])
        t_lin = np.asarray(t_eval)
